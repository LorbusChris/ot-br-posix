...
function signalIcon(scale) {
	...
	if (scale == 0)  return base + '/signal-000-000.svg';
	if (scale == 30) return base + '/signal-025-050.svg';
	if (scale == 60) return base + '/signal-050-075.svg';
	return base + '/signal-075-100.svg';
}

function renderSignal(bss) {
	var scale = signalScale(bss.link_quality_in);
	return E('abbr', { 'title': _('LinkQualityIn: %s').format(bss.link_quality_in) }, [
...
function renderNeighborTable(st) {
	...
	if (!st.neighbor || !st.neighbor.length) {
		...
		st.neighbor.forEach(function(bss) {
			var cells = isChild
				? [renderSignal(bss), bss.role, bss.rloc16, bss.age, bss.link_quality_in, bss.ext_address]
				: [renderSignal(bss), bss.role, bss.rloc16, bss.age, bss.avg_rssi, bss.last_rssi];
