...
function renderLeaderTable(st) {
	...
	return E('table', { 'class': 'table' }, [
		...
		E('tr', { 'class': 'tr cbi-rowstyle-2', 'style': 'border:solid 1px #ddd;border-top:hidden' }, [
			E('td', { 'class': 'td col-3 center' }, String(ld.leader_router_id ?? '')),
			E('td', { 'class': 'td col-3 center' }, String(ld.partitionid ?? '')),
			E('td', { 'class': 'td col-2 center' }, String(ld.weighting ?? '')),
			E('td', { 'class': 'td col-2 center' }, String(ld.data_version ?? '')),
			E('td', { 'class': 'td col-2 center' }, String(ld.stable_data_version ?? '')),
		])
	]);
}
...
function joinerActionsCell(j) {
	var eui64 = j.is_any ? '*' : j.eui64;
	return E('td', { 'class': 'td cbi-section-actions' },
		E('button', {
...
function renderNeighborsTable(neigh) {
	...
	(neigh.neighbor || []).forEach(function(bss) {
		var cells = isChild
			? [bss.rloc16, transRole(bss.role), bss.age, bss.link_quality_in, bss.ext_address]
			: [bss.rloc16, transRole(bss.role), bss.age, bss.avg_rssi, bss.last_rssi, bss.mode, bss.ext_address];
	...
	for (var i = 0; i < (neigh.joinernum || 0); i++) {
		...
		for (var k = 2; k < pendingCols; k++)
			cells.push(E('td', { 'class': 'td center' }, _('Pending')));
		cells.push(E('td', { 'class': 'td center' }, j.is_any ? '*' : (j.eui64 || '')));
