...
return view.extend({
	...
	render: function(data) {
		...
		function submit() {
			...
			if (!panid || isNaN(parseInt(channel))) {
				ui.addNotification(null, E('p', _('Missing network parameters, please join via the scan page.')), 'danger');
				return;
			}
			if (!/^[0-9a-f]+$/i.test(key) || key.length != 32) {
