	...
	render: function(data) {
					...
					E('div', { 'class': 'td cbi-section-actions' },
						E('button', {
							...
							'click': ui.createHandlerFn({}, function() {
								return callMacfilterRemove(addr, curMacState()).then(notify).then(refreshIfOk);
							})
						}, _('Remove')))
				]));
			});
		...
		function validateApply() {
			...
			if (isNaN(ch) || ch < 11 || ch > 26) {
				ui.addNotification(null, E('p', _('Channel must be a number between 11 and 26')), 'danger'); return false;
			}
			if (isNaN(Number(panidInput.value))) {
				ui.addNotification(null, E('p', _('PAN ID must be a number')), 'danger'); return false;
			}
			if (!/^[0-9a-f]+$/i.test(extpanidInput.value) || extpanidInput.value.length != 16) {
			...
			if (!/^[0-9a-f]+$/i.test(networkkeyInput.value) || networkkeyInput.value.length != 32) {
				ui.addNotification(null, E('p', _('Network key length must be 32 hex characters')), 'danger'); return false;
			}
			if (!/^[0-9a-f]+$/i.test(pskcInput.value) || pskcInput.value.length != 32) {
		...
		function doClear() {
			return callMacfilterClear().then(notify).then(refreshIfOk);
		}
		...
		function doAdd() {
			...
			return callMacfilterAdd(addr, curMacState()).then(notify).then(refreshIfOk);
		}
		...
		var macFilterDiv = E('div', { 'id': 'macfilterdiv', 'style': 'width:60%;margin-left:10%;display:none' }, [
			...
			E('div', { 'class': 'cbi-value', 'id': 'macfilterlistdiv', 'style': 'display:none' }, [].concat(
				(s.addrlist || []).length ? [
					E('div', { 'class': 'table' }, [
